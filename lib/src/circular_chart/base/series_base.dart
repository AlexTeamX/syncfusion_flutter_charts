...
class CircularSeriesBase {
  ...
  void _calculateStartAndEndAngle(
    ...
    int? _firstVisible;
    num? _gap;
    late RadialBarSeriesRendererExtension _seriesRenderer;
    final bool radialSeries = seriesRenderer.seriesType == 'radialbar';
    if (radialSeries) {
      _seriesRenderer = seriesRenderer as RadialBarSeriesRendererExtension;
      _seriesRenderer.innerRadius =
          seriesRenderer.segmentRenderingValues['currentInnerRadius']!;
    ...
    for (final ChartPoint<dynamic> point in seriesRenderer.renderPoints!) {
      if (point.isVisible) {
        if (radialSeries) {
          ...
          point.innerRadius = _seriesRenderer.innerRadius =
              _seriesRenderer.innerRadius +
                  ((point.index == _firstVisible) ? 0 : _ringSize!) -
